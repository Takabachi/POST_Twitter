...
class InfoPostsController extends Controller
{
    public function index() {
        ...
        if($user){
            $info_posts = InfoPost::orderBy('created_at','desc')->get();
    ...
    public function show(Request $request) {
        $info_post = InfoPost::find($request->id); //idで検索したレーコードを
        $user = \Auth::user(); //ログインユーザーの情報を取り出す
        if($user) {
            $login_user_id = $user->id;
        } else {
            $login_user_id = "";
        } 
        return view('show', ['info_post' => $info_post, 'login_user_id' => $login_user_id]); //viewに渡してreturnさせる
    }
